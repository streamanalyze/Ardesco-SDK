...
static int ard0011A_board_init(const struct device *dev)
{
	int err;

	err = ard0011A_magpio_configure();
	if (err) {
		LOG_ERR("ard0011A_magpio_configure failed with error: %d", err);
		return err;
	}
...
SYS_INIT(ard0011A_board_init, APPLICATION, CONFIG_APPLICATION_INIT_PRIORITY);
