...
static int ard0021B_magpio_configure(void)
{
#if defined(CONFIG_BSD_LIBRARY) && defined(CONFIG_NET_SOCKETS_OFFLOAD)
	int at_socket_fd;
	int buffer;
...
static int ard0021B_spi_init(struct device *dev)
{
	int ret;
	...
	gpio_out_dev = device_get_binding(DT_INST_0_NORDIC_NRF_SPIM_CS_GPIOS_CONTROLLER_0);
	if (!gpio_out_dev) {
		LOG_ERR("Cannot find %s!", DT_INST_0_NORDIC_NRF_SPIM_CS_GPIOS_CONTROLLER_0);
		return -ENODEV;
	}
...
static int ard0021B_sim_ctrl_init(void)
{
	int ret;
	struct device *gpio_out_dev;
	...
	if (!gpio_out_dev) {
		LOG_ERR("Cannot find %s!", BOARD_NS_GPIO_DEV_NAME);
		return -ENODEV;
	}

	ret = gpio_pin_configure(gpio_out_dev, BOARD_NS_SIM_SELECT_PIN, (GPIO_DIR_OUT));
	if (ret) {
		LOG_ERR("Error configuring pin %d!", BOARD_NS_SIM_SELECT_PIN);
	}
	bool use_esim = BOARD_NS_USE_ESIM;
	ret = gpio_pin_write(gpio_out_dev, BOARD_NS_SIM_SELECT_PIN, use_esim);
	if (ret) {
...
static int ard0021B_grove_ctrl_init(void)
{
	int ret;
	struct device *gpio_out_dev;
	...
	ret = gpio_pin_configure(gpio_out_dev, BOARD_NS_GROVE_SELECT_PIN, (GPIO_DIR_OUT));
	if (ret) {
		LOG_ERR("Error configuring pin %d!", BOARD_NS_GROVE_SELECT_PIN);
	}
	bool sel_digital = BOARD_NS_SEL_GROVEDIGITAL;
	ret = gpio_pin_write(gpio_out_dev, BOARD_NS_GROVE_SELECT_PIN, sel_digital);
	if (ret) {
...
static int ard0021B_board_init(struct device *dev)
{
	...
	err = ard0021B_sim_ctrl_init();
	if (err) {
		LOG_ERR("ard0021B_sim_ctrl_init failed with error: %d", err);
		return err;
	}

	err = ard0021B_magpio_configure();
	if (err) {
		LOG_ERR("ard0021B_magpio_configure failed with error: %d", err);
		return err;
	}

	err = ard0021B_grove_ctrl_init();
	if (err) {
		LOG_ERR("ard0021B_grove_ctrl_init failed with error: %d", err);
		return err;
	}
...
SYS_INIT(ard0021B_spi_init, POST_KERNEL, CONFIG_SPI_INIT_PRIORITY);
