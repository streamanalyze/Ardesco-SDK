 ...
 */ 
static int ard_ns_sim_ctrl_init(struct device *dev)
{
	int ret;
 ...
 */ 
static int ard_ns_grove_ctrl_init(struct device *dev)
{
	int ret;
 ...
 */ 
static int ard_ns_board_init(struct device *dev)
{
	int err;
 ...
 */ 
static int ard_ns_spi_init(struct device *dev)
{
	int ret, i;
	...
	struct device *gpio_dev = device_get_binding(BOARD_NS_GPIO_0_DEV_NAME);
	if (!gpio_dev) {
		LOG_ERR("Cannot find %s!", BOARD_NS_GPIO_0_DEV_NAME);
		return -ENODEV;
	}
